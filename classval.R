...
classifygroupprepropval <- function(groupfile, control, expnum){
  ...
  # cmnb <- confusionMatrix(final_predictionsnb$class, validation$class)
  ...
  # cmlvq <- confusionMatrix(final_predictionslvq$class, validation$class)
  ...
  # final_predictionsGbm <- predict.train(modelGbm$finalModel, validationdata)
  # saveRDS(final_predictionsGbm, file = glue("predGbm{expnum}.rds"))
  # cmGbm <- confusionMatrix(final_predictionsGbm$class, validation$class)
  ...
  # cmSvm <- confusionMatrix(final_predictionsSvm$class, validation$class)
